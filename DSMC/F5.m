...
for i = 2:length(t)
    % 计算加速度
    a_no_particles(i) = -(K / M) * x_no_particles(i-1) - (C / M) * v_no_particles(i-1);
    % 更新速度
...
for i = 2:length(t)
    % 计算结构的加速度
    a_particles_DSMC(i) = -(K / (M + n * m_p)) * x_particles_DSMC(i-1) - (C * C_DSMC / (M + n * m_p)) * v_particles_DSMC(i-1);
    ...
    a_particles_DSMC(i) = a_particles_DSMC(i) * 0.89 + perturbation_force(i) / (M + n * m_p); % 更新结构加速度
...
for i = 2:length(t)
    % 计算结构的加速度
    a_particles_DEM(i) = -(K / (M + n * m_p)) * x_particles_DEM(i-1) - (C * C_DEM / (M + n * m_p)) * v_particles_DEM(i-1);
    ...
    a_particles_DEM(i) = a_particles_DEM(i) * 0.9 + perturbation_force(i) / (M + n * m_p); % 更新结构加速度
